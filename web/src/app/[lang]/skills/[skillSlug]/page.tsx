...
export function generateStaticParams() {
  return SUPPORTED_LANGS.flatMap((lang) => getAllSkillMeta(lang).map((s) => ({ lang, skillSlug: s.slug })));
}
...
export default async function SkillPage({ params }: Props) {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6 md:py-16">
      ...
      <div className="mb-10">
        <h1 className="text-xl font-semibold tracking-tight text-gray-900">{skill.name}</h1>
        {skill.description && (
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-gray-500">{skill.description}</p>
        )}
